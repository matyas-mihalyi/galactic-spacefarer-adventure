...
export class SpacefarerHandler {
  ...
  static register(service: GalacticSpacefarerService) {
    ...
    service.before("UPDATE", Spacefarers, SpacefarerHandler.beforeSpacefarerUpdate);
    service.before("CREATE", Spacefarers, SpacefarerHandler.beforeSpacefarerCreate);
  }
  ...
  private static async beforeSpacefarerCreate(req: cds.Request) {
    ...
    if (!wormholeNavigationSkill) {
      req.data.wormholeNavigationSkill = await SpacefarerHandler.enhanceWormholeNavigationSkills();
    }
  }
  ...
  private static async validateRank(collectedStardust: number, rankId: string) {
    const desiredRank = await SELECT.one.from(SpacefarerHandler.entities.Ranks)
      .where`ID = ${rankId}`;
  ...
  private static async findHighestValidRank(
    ...
    return SELECT.one.from(SpacefarerHandler.entities.Ranks)
  ...
  private static async enhanceWormholeNavigationSkills(): Promise<number> {
    logger.log('Spacefarer candidate completed Wormhole Navigation Basic Training')
